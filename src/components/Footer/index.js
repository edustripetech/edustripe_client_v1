...
const Footer = () => {
  ...
  const showLinks = (string) => {
    return footerLink === string
      ? setFooterLink("none")
      : setFooterLink(string);
  };
  ...
  useEffect(() => {
    ...
    [...CompanyLinks, ...LegalLinks, ...ContactLinks].forEach((link) => {
      const classValue = Array.from(link.classList).find((value) =>
        ["company", "legal", "contact"].includes(value)
      );
        ...
        <div>
          <div className="footer-img">
            ...
            <div className="footer-socials">
              ...
              <a href={process.env.twitterUrl}>
                <Twitter />
              </a>
              <a href={process.env.linkedInUrl}>
                <Linkedin />
              </a>
              <a href={process.env.instagramUrl}>
                <Instagram />
              </a>
            </div>
          </div>
        ...
        <div className="footer-ul-div">
          <ul>
            <li className="head">
              Company{" "}
              <span
                onClick={() => showLinks("company")}
                className={`show-menu ${
                  footerLink === "company" ? "show-menu-company" : ""
                }`}
              >
                {" "}
                <RightSide />{" "}
          ...
          <ul>
            <li className="head">
              Legal{" "}
              <span
                onClick={() => showLinks("legal")}
                className={`show-menu ${
                  footerLink === "legal" ? "show-menu-legal" : ""
                }`}
              >
                <RightSide />
              </span>
          ...
          <ul id="contactUs">
            <li className="head">
              Contact{" "}
              <span
                onClick={() => showLinks("contact")}
                className={`show-menu ${
                  footerLink === "contact" ? "show-menu-contact" : ""
                }`}
              >
                <RightSide />
              </span>
            </li>
            <li className="contact contact-hide">
              {process.env.addressInfo || "294 Herbert Macaulay,"}
            </li>
            <li className="contact contact-hide">
              {process.env.addressCityState || "Yaba, Lagos,"}
            </li>
            <li className="contact contact-hide">
              {process.env.addressCountry || "Nigeria"}
            </li>
            <li className="phoneNumber contact contact-hide">
              {process.env.phoneNumber || "08025129751"}
            </li>
            <li className="contact contact-hide">
              {process.env.email || "info@edustripe.com"}
            </li>
          </ul>
        </div>
      </div>
